...
public class ItemManagerUtil {
    public  <T extends Item> T getItemIfExist(Long id, ItemBaseRepository<T> repository){
    ...
    public  <T extends Item> List<T> getListOfItems(ItemBaseRepository<T> repository){
        List<T> items = repository.findAll();
        return items;
    }
    public  <T extends Item,E extends ItemDto> List<E> fromItemToItemDto(List<T> list, E itemDto){
    ...
    public  <T extends  Item, E extends ItemForm> T createItem(T item, E itemForm){
